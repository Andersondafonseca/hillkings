...
import {renderPage,googleAnalyticsCspHash} from './presentation.mjs';
...
function securityHeaders(res){
 ...
 res.setHeader('Content-Security-Policy',`default-src 'self'; script-src 'self' ${googleAnalyticsCspHash} https://www.googletagmanager.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:${cloud} https://*.google-analytics.com https://www.googletagmanager.com; font-src 'self'; connect-src 'self'${cloud} https://*.google-analytics.com https://*.analytics.google.com https://www.googletagmanager.com; media-src 'self' blob:${cloud}; object-src 'none'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'`);
...
export async function handler(req,res){
 ...
 try{
  ...
  if(catalog.mode==='catalog-only'){
   html=html.replace('<body',`<body data-catalog-mode="catalog-only"`);
   if(p==='/admin')html=html.replace(/(<body[^>]*>)/,'$1<div class="deployment-notice" role="status">Painel ainda não conectado ao Supabase. Configure o banco para ativar o acesso e salvar alterações.</div>');
  }
